...
[
    ...
    const { cycleOpts, makeServer } = server_helpers_1.makeMakeServer({ proto, version });
    ...
    describe("basic", () => {
        ...
        describe(`generic (${protoVersion})`, () => {
            it("should be able to perform simple GET", async () => {
                ...
                const versionNumber = parseInt(version.substr(version.length - 1), 10);
            ...
            it("should be able to get upper-case headers", async () => {
                const { server, port } = await makeServer();
                const json = { foo: "bar" };
                const response = await fetch(`${proto}//localhost:${port}/echo`, {
                    json,
                    method: "POST",
                });
            ...
            it("should trigger onTrailers", async () => {
                ...
                const deferredTrailers = already_1.defer();
                const onTrailers = deferredTrailers.resolve;
                const response = await fetch(`${proto}//localhost:${port}/trailers`, {
                    json: trailers,
                    method: "POST",
                    onTrailers,
                });
            ...
            it("should timeout on a slow request", async () => {
                ...
                const { server, port } = await makeServer();
                const eventualResponse = fetch(`${proto}//localhost:${port}/wait/20`, {
                    method: "POST",
                    timeout: 8,
                });
                const err = await getRejection(eventualResponse);
            ...
            it("should not timeout on a fast request", async () => {
                const { server, port } = await makeServer();
                const response = await fetch(`${proto}//localhost:${port}/wait/1`, {
                    method: "POST",
                    timeout: 100,
                });
            ...
            it("should be able to POST large (16MiB) stream with known length", async () => {
            ...
            it("should be able to POST large (16MiB) stream with unknown length", async () => {
                jest.setTimeout(2000);
                const { server, port } = await makeServer();
                const chunkSize = 1024 * 1024;
                const chunks = 16;
